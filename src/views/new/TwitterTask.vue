...
<template>
    <Form ref="newForm" :model="formData" :label-width="100" :rules="ruleValidate" style="padding:20px;">
        ...
        <FormItem label="推文地址"  prop="reply_url" :class="this.$route.query.type!='create' ? 'show': 'hidden'">
            <Input v-model="formData.reply_url" placeholder="请输入twitter主题地址"></Input>
        </FormItem>
        <FormItem>
    ...
    export default {
        data () {
            return {
                ...
                urls: {
                    "create": {"url":"/task/create.html","type":1},
                    "reply": {"url":"/task/reply_twitter.html","type":2},
                    "forward": {"url":"/task/forward_twitter.html","type":3}
                }
            }
        },
        mounted() {
            ...
            handleSubmit (name) {
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        ...
                        data.append('type', this.urls[type].type)
                        ...
                        this.axios.post(this.urls[type].url, data).then((response)=>{
